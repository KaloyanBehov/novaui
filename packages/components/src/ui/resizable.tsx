...
const ResizablePanel = React.forwardRef<React.ElementRef<typeof View>, ResizablePanelProps>(
  ({ className, defaultSize, minSize, maxSize, ...props }, ref) => {
    ...
    return <View ref={ref} className={cn('overflow-hidden', className)} style={style} {...props} />;
  }
);
...
const ResizableHandle = React.forwardRef<React.ElementRef<typeof View>, ResizableHandleProps>(
    ...
    return (
      <GestureDetector gesture={gesture}>
        <View
          ...
          collapsable={false}
          className={cn(
            'relative z-10 flex items-center justify-center bg-transparent',
            isHorizontal ? '-mx-4 h-full w-8' : '-my-4 h-8 w-full',
            className
          )}
          style={[isHorizontal ? { minWidth: 32 } : { minHeight: 32 }]}
          {...props}>
          <View
            pointerEvents="none"
            className={cn(
              ...
              isHorizontal
                ...
                : 'absolute top-1/2 right-0 left-0 h-px'
            )}
          ...
          {withHandle && (
            <View
              pointerEvents="none"
              className="bg-muted border-border absolute z-20 flex h-6 w-5 items-center justify-center rounded border">
              {isHorizontal ? (
