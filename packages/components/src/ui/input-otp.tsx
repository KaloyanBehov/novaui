...
>(({ className, containerClassName, value, onChangeText, maxLength, children, ...props }, ref) => {
  ...
  return (
    <View className={cn('relative flex items-center gap-2', containerClassName)}>
      <TextInput
        ref={inputRef}
        value={value}
        ...
        {...props}
      />
      <Pressable onPress={handlePress} className={cn('flex-row items-center gap-2', className)}>
...
>(({ index, className, otpValue = '', otpMaxLength, otpIsFocused, ...props }, ref) => {
  ...
  const isActive =
    otpIsFocused && index === otpValue.length && otpValue.length < (otpMaxLength || 0);

  return (
    <View
      ref={ref}
      className={cn(
        'border-input bg-background relative flex h-10 w-10 items-center justify-center border text-sm',
      ...
      {isActive && (
        <View className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <View className="bg-foreground h-4 w-px" />
        </View>
      )}
    </View>
  );
