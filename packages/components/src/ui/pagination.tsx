...
const PaginationLink = React.forwardRef<React.ElementRef<typeof Pressable>, PaginationLinkProps>(
  ({ className, isActive, size = 'icon', children, ...props }, ref) => (
    <Pressable
      ...
      {typeof children === 'string' ? (
        <Text className={cn(isActive ? 'text-foreground' : 'text-muted-foreground')}>
          {children}
        </Text>
      ) : (
        children
...
const PaginationEllipsis = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof View> & { className?: string }
>(({ className, ...props }, ref) => (
  <View ref={ref} className={cn('flex h-9 w-9 items-center justify-center', className)} {...props}>
